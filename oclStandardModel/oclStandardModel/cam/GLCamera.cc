...
// the 3D point there.  If there's nothing there, returns false.
bool GLCamera::read_depth(int x, int y, point &p) const
{
	...
	float d;
	glReadPixels(xmin + x, ymin + y, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &d);
	if (d > 0.0001f && d < 0.9999f) {
		GLdouble X, Y, Z;
		gluUnProject(xmin + x, ymin + y, d, M, P, V, &X, &Y, &Z);
		p = point(X, Y, Z);
		return true;
	} else {
		return false;
...
void GLCamera::setupGL(const point &scene_center, float scene_size) const
{
	...
	glFrustum(w * tile_l/2.0f, w * tile_r/2.0f, h * tile_d/2.0f, h * tile_u/2.0f, neardist, fardist);
